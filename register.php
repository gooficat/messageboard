...
<html>
	...
	<body>
       ...
       if ($_SERVER["REQUEST_METHOD"] == "POST") {
           ...
           $register_check_exist_query = $db->prepare(
           ...
           if ($register_check_exist_query->execute()->fetchArray()) {
               ...
               $register_check_exist_query_username = $db->prepare(
                   "SELECT * FROM users WHERE username = ?",
               );
               $register_check_exist_query_username->bindValue(
                   1,
                   $_POST["username"],
               );
               if (
                   $register_check_exist_query_username->execute()->fetchArray()
               ) {
